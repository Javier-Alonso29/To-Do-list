...
function run(task) {
  ...
  if (task.timer === "00:00") {
    ...
    var current_time = task.timer.split(":");
...
function loadTasks() {
  ...
  array_tasks.forEach((element) => {
    if (element.status == "stop") {
      block +=
        "<tr>" +
        "<td class='th-information'>" +
        element.description +
        "</td>" +
        "<td class='th-edit'><a class='btn-a btn-success' onclick='editTask(" +
        JSON.stringify(element) +
        ")'><i class='far fa-edit'></i></a></td>" +
        "<td class='th-action'><a id='btn-time" +
        element.id +
        "' class='btn-a btn-blue' onclick='timer(" +
        JSON.stringify(element) +
        ")'><i class='far fa-play-circle'></i></a></td>" +
        "<td class='th-timer' id='elementTime" +
        element.id +
        "'>" +
        element.timer +
        "</td>" +
        "<td class='th-quit'><a class='btn-a btn-red' onclick='deleteTask(" +
        JSON.stringify(element) +
      ...
      block +=
        ...
        element.description +
        "</td>" +
        "<td class='th-edit'><a class='btn-a disable' onclick='editTask(" +
        JSON.stringify(element) +
        ")'><i class='far fa-edit'></i></a></td>" +
        "<td class='th-action'><a id='btn-time" +
        element.id +
        "' class='btn-a btn-red' onclick='timer(" +
        JSON.stringify(element) +
        ")'><i class='far fa-pause-circle'></i></a></td>" +
        "<td class='th-timer' id='elementTime'><a class='btn-a running'>Runing</a></td>" +
        "<td class='th-quit'><a class='btn-a disable' onclick='deleteTask(" +
        JSON.stringify(element) +
        ")'><i class='far fa-window-close'></i></a></td>" +
        "</tr>";
    }
